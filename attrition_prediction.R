...
df$RANGE.PENETRATION[df$RANGE.PENETRATION >400] <- mean(df$RANGE.PENETRATION)
...
df <- df %>% 
  ...
  filter(JOB.GRADE != "10" & Type.of.Contract == "localcontract" & MainLoc == "Vietnam") %>%
  filter(Transfer == 0 & Leaverin3months == 0 & MID != "NA")
df$CC1 <- as.factor(gsub(" ", "", df$CC1))
df$CC2 <- as.factor(gsub(" ", "", df$CC2))
df$CC3 <- as.factor(gsub(" ", "", df$CC3))
# SELECT only variables for prediction
df <- df %>% 
  select(CC1, Length.of.Service, JOB.GRADE, 
...
ggplot(data = df, aes(x = RANGE.PENETRATION.2, ..count..)) + geom_bar(aes(fill = Leaver.2), position = 'fill')
